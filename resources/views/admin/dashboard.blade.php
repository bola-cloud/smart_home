...
<div class="container-fluid py-4">
    ...
    <div class="card shadow-lg p-4">
        ...
        <form action="{{ route('admin.dashboard') }}" method="GET" class="mb-4">
            <div class="row">
                <div class="col-md-6">
                    <label for="from_date" class="form-label fw-bold">{{ __('dashboard.from_date') }}</label>
                    <input type="date" name="from_date" id="from_date" class="form-control" value="{{ request('from_date') }}">
                </div>
                <div class="col-md-6">
                    <label for="to_date" class="form-label fw-bold">{{ __('dashboard.to_date') }}</label>
                    <input type="date" name="to_date" id="to_date" class="form-control" value="{{ request('to_date') }}">
                </div>
            </div>
            <div class="mt-3 text-end">
                <button type="submit" class="btn btn-primary px-4">{{ __('dashboard.filter') }}</button>
            </div>
        </form>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-4">
                <div class="card bg-primary shadow-sm text-center p-4 text-white" style="height: 180px;">
                    <h5 class="fw-bold">{{ __('dashboard.total_devices') }}</h5>
                    <h2 class="fw-bold">{{ $totalDevices }}</h2>
                    <p>{{ __('dashboard.active') }}: {{ $activeDevices }} | {{ __('dashboard.inactive') }}: {{ $inactiveDevices }}</p>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card bg-success shadow-sm text-center p-4 text-white" style="height: 180px;">
                    <h5 class="fw-bold">{{ __('dashboard.purchased_products') }}</h5>
            ...
            <div class="col-md-4">
                <div class="card bg-danger shadow-sm text-center p-4 text-white" style="height: 180px;">
                    <h5 class="fw-bold">{{ __('dashboard.total_income') }}</h5>
            ...
            <div class="col-md-4">
                <div class="card bg-info shadow-sm text-center p-4 text-white" style="height: 180px;">
                    <h5 class="fw-bold">{{ __('dashboard.users') }}</h5>
            ...
            <div class="col-md-4">
                <div class="card bg-warning shadow-sm text-center p-4 text-dark" style="height: 180px;">
                    <h5 class="fw-bold">{{ __('dashboard.device_types') }}</h5>
